...
import ru.dins.scalashool.imageboard.models.ResponseModels.{ApiError, ImageResponse, ImageUpload, SuccessCreation}
...
trait TapirImageAdapter[F[_]] {
  def getImage(id: Long): F[Either[ApiError, ImageResponse]]
...
object TapirImageAdapter {
  ...
  def apply[F[_]: ContextShift : Sync](storage: PostgresStorage[F], modelConverter: ModelConverter[F]) = new TapirImageAdapter[F] {
    ...
    override def getImage(id: Long): F[Either[ApiError, ImageResponse]] =  storage.getImage(id).flatMap{
